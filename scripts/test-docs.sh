...
# Test 45: All non-index doc pages have 7+ admonitions
echo ""
echo "Test 45: Checking all non-index doc pages have 7+ admonitions..."
...
for file in $(find "$ROOT_DIR/docs" -name "*.md" ! -name "index.md" 2>/dev/null); do
    ...
    if [ "$admonition_count" -lt 7 ]; then
        rel_file="${file#$ROOT_DIR/}"
        echo "  Only $admonition_count admonition(s) in $rel_file (need 7+)"
        DOC_ADMONITION_ISSUES=$((DOC_ADMONITION_ISSUES + 1))
    fi
done

if [ "$DOC_ADMONITION_ISSUES" -eq 0 ]; then
    pass_test "All non-index doc pages have 7+ admonitions"
else
    fail_test "Found $DOC_ADMONITION_ISSUES doc page(s) with fewer than 7 admonitions" "Add admonitions (:::tip, :::note, :::warning) to doc pages"
fi
...
# Test 47: All non-index doc pages have 5+ Mermaid diagrams
DOC_MERMAID2_ISSUES=0
echo "Test 47: Checking all non-index doc pages have 5+ Mermaid diagrams..."
for file in $(find "$ROOT_DIR/docs" -name "*.md" ! -name "index.md" 2>/dev/null); do
    mermaid_count=$(grep -c '```mermaid' "$file" 2>/dev/null) || mermaid_count=0
    if [ "$mermaid_count" -lt 5 ]; then
...
if [ "$DOC_MERMAID2_ISSUES" -eq 0 ]; then
    pass_test "All non-index doc pages have 5+ Mermaid diagrams"
else
    fail_test "Found $DOC_MERMAID2_ISSUES non-index doc page(s) with fewer than 5 Mermaid diagrams" "Add Mermaid diagrams to doc pages for visual explanations"
fi
