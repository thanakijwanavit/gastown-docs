...
# Test 37: All doc pages have 2+ blog post cross-links
echo ""
echo "Test 37: Checking all doc pages have 2+ blog post cross-links..."
...
if [ "$BLOG_CROSSLINK_ISSUES" -eq 0 ]; then
    pass_test "All doc pages have 2+ blog post cross-links"
else
...
# Test 45: All non-index doc pages have 3+ admonitions
echo ""
echo "Test 45: Checking all non-index doc pages have 3+ admonitions..."
...
for file in $(find "$ROOT_DIR/docs" -name "*.md" ! -name "index.md" 2>/dev/null); do
    ...
    if [ "$admonition_count" -lt 3 ]; then
        rel_file="${file#$ROOT_DIR/}"
        echo "  Only $admonition_count admonition(s) in $rel_file (need 3+)"
        DOC_ADMONITION_ISSUES=$((DOC_ADMONITION_ISSUES + 1))
    fi
done

if [ "$DOC_ADMONITION_ISSUES" -eq 0 ]; then
    pass_test "All non-index doc pages have 3+ admonitions"
else
...
# Test 51: All blog posts have at least one admonition
BLOG_ADMONITION_ISSUES=0
echo "Test 51: Checking all blog posts have at least one admonition..."
for file in $(find "$ROOT_DIR/blog" -name "*.md" 2>/dev/null); do
    admonition_count=$(grep -cE '^:::(tip|note|warning|info|danger|caution)' "$file" 2>/dev/null) || admonition_count=0
    if [ "$admonition_count" -lt 1 ]; then
        echo "  WARNING: $(basename "$file") has no admonitions"
        BLOG_ADMONITION_ISSUES=$((BLOG_ADMONITION_ISSUES + 1))
    fi
done
if [ "$BLOG_ADMONITION_ISSUES" -eq 0 ]; then
    pass_test "All blog posts have at least one admonition"
else
