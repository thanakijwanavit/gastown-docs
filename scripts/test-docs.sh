...
# Test 39: Blog posts link to at least 8 other blog posts
echo ""
...
for file in $(find "$ROOT_DIR/blog" -name "*.md" 2>/dev/null); do
    ...
    if [ "$blog_link_count" -lt 8 ]; then
        rel_file="${file#$ROOT_DIR/}"
        echo "  Only $blog_link_count blog link(s) in $rel_file (need 8+)"
        BLOG_INTERLINK_ISSUES=$((BLOG_INTERLINK_ISSUES + 1))
    fi
done

if [ "$BLOG_INTERLINK_ISSUES" -eq 0 ]; then
    pass_test "All blog posts link to 8+ other blog posts"
else
    fail_test "Found $BLOG_INTERLINK_ISSUES blog post(s) with fewer than 8 blog cross-links" "Add links to related blog posts in the Next Steps section"
fi
...
# Test 43: All blog posts have 8+ Mermaid diagrams
echo ""
echo "Test 43: Checking all blog posts have 8+ Mermaid diagrams..."
BLOG_MULTI_MERMAID_ISSUES=0

for file in $(find "$ROOT_DIR/blog" -name "*.md" 2>/dev/null); do
    mermaid_count=$(grep -c '```mermaid' "$file" 2>/dev/null) || mermaid_count=0

    if [ "$mermaid_count" -lt 8 ]; then
        rel_file="${file#$ROOT_DIR/}"
        echo "  Only $mermaid_count diagram(s) in $rel_file (need 8+)"
        BLOG_MULTI_MERMAID_ISSUES=$((BLOG_MULTI_MERMAID_ISSUES + 1))
    fi
done

if [ "$BLOG_MULTI_MERMAID_ISSUES" -eq 0 ]; then
    pass_test "All blog posts have 8+ Mermaid diagrams"
else
    fail_test "Found $BLOG_MULTI_MERMAID_ISSUES blog post(s) with fewer than 8 Mermaid diagrams" "Add at least 8 Mermaid diagrams to each blog post"
fi

# Test 44: All blog posts have 8+ /docs/ cross-links
echo ""
echo "Test 44: Checking all blog posts have 8+ /docs/ cross-links..."
BLOG_DOCS_LINK_ISSUES=0

for file in $(find "$ROOT_DIR/blog" -name "*.md" 2>/dev/null); do
    docs_link_count=$(grep -co '/docs/' "$file" 2>/dev/null) || docs_link_count=0

    if [ "$docs_link_count" -lt 8 ]; then
        rel_file="${file#$ROOT_DIR/}"
        echo "  Only $docs_link_count /docs/ link(s) in $rel_file (need 8+)"
        BLOG_DOCS_LINK_ISSUES=$((BLOG_DOCS_LINK_ISSUES + 1))
    fi
done

if [ "$BLOG_DOCS_LINK_ISSUES" -eq 0 ]; then
    pass_test "All blog posts have 8+ /docs/ cross-links"
else
    fail_test "Found $BLOG_DOCS_LINK_ISSUES blog post(s) with fewer than 8 /docs/ links" "Add /docs/ cross-links to blog posts for better navigation"
fi

# Test 45: All non-index doc pages have 8+ admonitions
echo ""
echo "Test 45: Checking all non-index doc pages have 8+ admonitions..."
...
for file in $(find "$ROOT_DIR/docs" -name "*.md" ! -name "index.md" 2>/dev/null); do
    ...
    if [ "$admonition_count" -lt 8 ]; then
        rel_file="${file#$ROOT_DIR/}"
        echo "  Only $admonition_count admonition(s) in $rel_file (need 8+)"
        DOC_ADMONITION_ISSUES=$((DOC_ADMONITION_ISSUES + 1))
    fi
done

if [ "$DOC_ADMONITION_ISSUES" -eq 0 ]; then
    pass_test "All non-index doc pages have 8+ admonitions"
else
    fail_test "Found $DOC_ADMONITION_ISSUES doc page(s) with fewer than 8 admonitions" "Add admonitions (:::tip, :::note, :::warning) to doc pages"
fi
...
# Test 47: All non-index doc pages have 6+ Mermaid diagrams
DOC_MERMAID2_ISSUES=0
echo "Test 47: Checking all non-index doc pages have 6+ Mermaid diagrams..."
for file in $(find "$ROOT_DIR/docs" -name "*.md" ! -name "index.md" 2>/dev/null); do
    mermaid_count=$(grep -c '```mermaid' "$file" 2>/dev/null) || mermaid_count=0
    if [ "$mermaid_count" -lt 6 ]; then
...
if [ "$DOC_MERMAID2_ISSUES" -eq 0 ]; then
    pass_test "All non-index doc pages have 6+ Mermaid diagrams"
else
    fail_test "Found $DOC_MERMAID2_ISSUES non-index doc page(s) with fewer than 6 Mermaid diagrams" "Add Mermaid diagrams to doc pages for visual explanations"
fi
...
# Test 51: All blog posts have at least 7 admonitions
BLOG_ADMONITION_ISSUES=0
echo "Test 51: Checking all blog posts have 7+ admonitions..."
for file in $(find "$ROOT_DIR/blog" -name "*.md" 2>/dev/null); do
    admonition_count=$(grep -cE '^:::(tip|note|warning|info|danger|caution)' "$file" 2>/dev/null) || admonition_count=0
    if [ "$admonition_count" -lt 7 ]; then
...
if [ "$BLOG_ADMONITION_ISSUES" -eq 0 ]; then
    pass_test "All blog posts have 7+ admonitions"
else
    fail_test "Found $BLOG_ADMONITION_ISSUES blog post(s) with fewer than 7 admonitions" "Add :::tip, :::note, or :::warning admonitions to blog posts"
fi
