...
# Test 47: All non-index doc pages have 3+ Mermaid diagrams
DOC_MERMAID2_ISSUES=0
echo "Test 47: Checking all non-index doc pages have 3+ Mermaid diagrams..."
for file in $(find "$ROOT_DIR/docs" -name "*.md" ! -name "index.md" 2>/dev/null); do
    mermaid_count=$(grep -c '```mermaid' "$file" 2>/dev/null) || mermaid_count=0
...
if [ "$DOC_MERMAID2_ISSUES" -eq 0 ]; then
    pass_test "All non-index doc pages have 3+ Mermaid diagrams"
else
    fail_test "Found $DOC_MERMAID2_ISSUES non-index doc page(s) with fewer than 3 Mermaid diagrams" "Add Mermaid diagrams to doc pages for visual explanations"
fi
