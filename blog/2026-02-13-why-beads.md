...
Every project management tool assumes a human is driving. Jira has dashboards designed for people to click through. GitHub Issues has a web UI built for browser interactions. Linear optimizes for keyboard-driven humans. None of them are designed for an AI agent that works through a terminal. Understanding how [boot dogs](/blog/boot-dogs) initialize agents helps clarify why agent-native tooling matters.
...
The `bd` command routes automatically based on prefix. Each [rig](/docs/concepts/rigs) has its own prefix configured during setup:
...
Here's how a typical polecat uses Beads during a work session. This workflow is part of a larger execution pattern managed by the [daemon process](/blog/daemon-process) that coordinates all agent activity.

```mermaid
stateDiagram-v2
    direction LR
    [*] --> Ready: bd create
    Ready --> InProgress: bd update --status=in_progress
    InProgress --> Testing: Agent runs tests
    Testing --> FollowUp: bd create (new issue discovered)
    Testing --> Done: bd close
    Done --> [*]
    FollowUp --> Ready: New bead enters queue
    Note right of Done: Commit hash in --reason
```
...
This diagram shows how Beads integrates with traditional project management tools in a typical Gas Town deployment.

```mermaid
flowchart LR
    subgraph Human["Human PM Layer"]
        JIRA[Jira/Linear<br/>Sprint Planning]
        DASH[Dashboards<br/>Reporting]
    end
    subgraph Agent["Agent Execution Layer"]
        BEADS[Beads<br/>CLI-First Tracking]
        AGENTS[Polecats<br/>Execute Work]
    end
    JIRA -->|Mayor reads| BEADS
    BEADS -->|Export for visibility| DASH
    BEADS --> AGENTS
```


:::info Export Beads to External Trackers for Stakeholder Visibility
While Beads lacks a web UI, you can periodically export bead data to your human-facing tracker using the `bd list --json` command and a simple sync script. This keeps stakeholders informed of Gas Town's execution progress without requiring them to learn CLI commands or lose their familiar dashboards. Schedule the export to run daily or after major convoy completions.
:::
...
- **[Understanding Rigs](/blog/understanding-rigs)** — How rigs organize beads and their execution workspaces
- [GUPP Principles](/docs/concepts/gupp) — How beads enforce forward-only state transitions
