...
This matters because Gas Town uses dependencies to sequence agent work. The Mayor won't sling `ga-002` until `ga-001` has merged.
...
In a multi-rig Gas Town setup, beads are distributed across rigs. Each rig has its own beads database with a unique prefix:
...
This means agents can reference and update beads from any rig without knowing the physical database location.
