...
:::info Gas Town requires no changes to your existing codebase
Gas Town operates entirely outside your project's source tree. It uses its own git clones, its own merge queue, and its own communication layer. You do not need to add configuration files, SDKs, or plugins to your project to start using it.
:::
...
| Concept | Role | Category | Description |
|---------|------|----------|-------------|
| **Rig** | Project environment | Infrastructure | An isolated workspace with its own git clone, merge queue, and worker pool. Each project gets its own rig. |
| **Polecat** | Transient worker agent | Agents | A short-lived AI coding agent that picks up a bead, executes the work, and terminates when done. The primary unit of parallel execution. |
| **Witness** | Agent supervisor | Supervision | Monitors running polecats for crashes, stalls, and failures. Restarts or escalates when something goes wrong. |
| **Refinery** | Merge queue | Infrastructure | Serializes parallel agent output into clean, validated commits on main. Ensures no merge conflicts or broken builds. |
| **Bead** | Work item | Work Tracking | A lightweight issue or task, stored in Dolt alongside your code. Beads are what agents pick up and work on. |
| **Convoy** | Batch tracker | Work Tracking | Groups multiple beads into a single trackable unit. Auto-closes when all beads in the batch complete. |
| **Molecule** | Workflow instance | Execution | A live, running workflow created by "pouring" a formula. Tracks which steps are done, in progress, or pending. |
| **Mayor** | Top-level orchestrator | Supervision | Decomposes high-level intents into beads, manages convoys, and oversees the entire agent hierarchy. |
...
The following diagram illustrates the supervision hierarchy that ensures reliability across the agent fleet.

```mermaid
graph TD
    DAEMON[Daemon: Go Process] --> DEACON[Deacon: Town Health]
    DEACON --> W1[Witness: Rig 1]
    DEACON --> W2[Witness: Rig 2]
    DEACON --> W3[Witness: Rig N]
    W1 --> P1[Polecats: Workers]
    W2 --> P2[Polecats: Workers]
    W3 --> P3[Polecats: Workers]
    DEACON --> BOOT[Boot: Triage]
    BOOT -.->|monitors| DEACON
```
...
:::warning Start with Good Test Coverage
Gas Town agents validate their own work by running your test suite. If your test coverage is low or your tests are unreliable, agents will land broken code on main or waste tokens retrying flaky failures. Invest in solid test infrastructure before scaling beyond a single agent -- it pays for itself many times over in reduced debugging and rework.
:::

```mermaid
stateDiagram-v2
    [*] --> Install: gt install
    Install --> Configure: gt rig create
    Configure --> SingleAgent: Run 1 polecat
    SingleAgent --> MultiAgent: Scale to 3-5
    MultiAgent --> FullFleet: Scale to 10-30
    FullFleet --> Autonomous: Mayor-driven
    Autonomous --> [*]
```

:::tip Start Small and Scale Gradually
Most successful Gas Town deployments begin with a single rig and two or three polecats. This lets you validate your test suite, learn the supervision hierarchy, and build confidence before scaling to a full fleet. The [Eight Stages](/blog/eight-stages-self-assessment) guide helps you identify your current maturity level and plan the next step.
:::

```mermaid
graph LR
    subgraph Communication["Agent Communication Layer"]
        MAIL[Mail: Async Messages]
        NUDGE[Nudge: Wake Signals]
        HOOK[Hook: Work Assignment]
    end
    subgraph Storage["Persistence Layer"]
        BEADS[Beads: Issue Tracking]
        DOLT[Dolt: Versioned DB]
    end
...
sequenceDiagram
    ...
    H->>M: Describe intent
    M->>B: Create beads + convoy
    M->>P: Sling beads to workers
    P->>P: Execute in parallel
    P->>R: Submit merge requests
    R->>R: Rebase + validate + test
    R->>G: Fast-forward merge
    Note over G: Clean linear history
```
...
For human developers working alongside agents, the [crew workflow](/blog/crew-workflow) provides persistent workspaces and direct push access to main.

:::info You Don't Need 30 Agents to Benefit
Even with 2-3 concurrent polecats, Gas Town's Refinery and Witness prevent the coordination headaches of running agents ad-hoc. The system scales smoothly from a single agent to a full fleet, so start small and grow as you learn the patterns.
:::

You don't need to be running 30 agents to benefit. Even with 2-3 concurrent workers, Gas Town's Refinery and Witness dramatically reduce the manual coordination overhead compared to running agents ad-hoc. For a complete reference of all available commands, see the [CLI reference](/docs/cli-reference/town).
...
:::caution Backup Your Work Before First Convoy
Before running your first convoy with real agents working in parallel, ensure you have a clean git history and push all important work. While Gas Town's merge queue prevents conflicts, the first convoy is the best time to learn the system's behavior without risking critical work in progress.
:::
...
### Gas Town Component Interactions

The following diagram illustrates how the major Gas Town components interact during a typical work cycle from bead creation to merge completion.

```mermaid
sequenceDiagram
    participant H as Human
    participant BD as Beads System
    ...
    H->>BD: Create bead
    H->>PC: Sling to polecat
    PC->>PC: Execute work
    W->>PC: Monitor health
    PC->>RF: Submit to merge queue
    RF->>RF: Validate and test
    RF->>M: Merge to main
    M-->>H: Work complete
```
...
- [Why Beads?](/blog/why-beads) — Understanding the bead-based issue tracking system that powers Gas Town workflows
