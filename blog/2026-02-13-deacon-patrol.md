...
[Boot](/docs/agents/boot) dogs are deliberately short-lived. They assess and report, then exit. The Deacon takes action based on Boot's assessment. This separation keeps triage lightweight and prevents the Deacon from getting stuck doing detailed investigation. For more details on how Boot operates, see [Boot Dogs: The Daemon's Triage Engine](/blog/boot-dogs).
