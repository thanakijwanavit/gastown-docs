...
- **Crashes cannot lose work.** If an agent crashes mid-task, all completed steps are persisted. A fresh agent resumes from the last checkpoint.
...
The molecule tracks step completion in the beads database. When a fresh polecat picks up the work, it reads the molecule state and continues from exactly where the crashed agent left off.
