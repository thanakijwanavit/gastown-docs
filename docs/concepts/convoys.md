...
When the Mayor receives a complex request -- "Fix the auth module, add email validation, and update the docs" -- that becomes three separate beads. Without convoys, tracking which of those three are done, which are in progress, and when the entire batch has landed would require manual checking.
...
This works because convoys support **cross-prefix tracking** -- the convoy bead stores full IDs including their prefix, allowing it to query status from any rig's `.beads/` database.
...
This is useful for the Mayor to identify work that needs to be slung to polecats.
